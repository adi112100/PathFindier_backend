...
def djsktras_initilaize_path(row,col):
    return [[] for i in range(row*col)]


def djsktras_creategraph(row, col, walls):
...
def djsktras_algorithm(row, col, start, end, walls):
    visited_path = [0 for i in range(row*col)]
    shortest_path = djsktras_initilaize_path(row, col)
    shortest_path_val = [math.inf for i in range(row*col)]
    
    adjlst = djsktras_creategraph(row, col, walls)
        ...
        if i==0:
            ...
            if shortest_path_val[end]!=math.inf:
                ...
                for j in range(totalnodes):
                    if visited_path[j]==0 and shortest_path_val[j] < minnval:
